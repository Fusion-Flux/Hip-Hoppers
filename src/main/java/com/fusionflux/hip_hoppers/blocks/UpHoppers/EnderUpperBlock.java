...
public class EnderUpperBlock extends BlockWithEntity {
    ...
    public EnderUpperBlock(Settings settings, int itemMoveSpeed) {
        super(settings);
    ...
    public BlockEntity createBlockEntity(BlockPos pos, BlockState state) {
        return new EnderUpperBlockEntity(pos, state,this.transferSpeed);
    }

    @Nullable
    public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        return world.isClient ? null : checkType(type, HipHoppersBlocks.ENDER_UPPER_BLOCK_ENTITY, EnderUpperBlockEntity::serverTick);
    }

    public void onPlaced(World world, BlockPos pos, BlockState state, LivingEntity placer, ItemStack itemStack) {
        if (itemStack.hasCustomName()) {
            BlockEntity blockEntity = world.getBlockEntity(pos);
            if (blockEntity instanceof EnderUpperBlockEntity) {
                ((EnderUpperBlockEntity)blockEntity).setCustomName(itemStack.getName());
            }
        }
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        if (world.isClient) {
            ...
            BlockEntity blockEntity = world.getBlockEntity(pos);
            if (blockEntity instanceof EnderUpperBlockEntity) {
                player.openHandledScreen((EnderUpperBlockEntity)blockEntity);
    ...
    public void onStateReplaced(BlockState state, World world, BlockPos pos, BlockState newState, boolean moved) {
        if (!state.isOf(newState.getBlock())) {
            BlockEntity blockEntity = world.getBlockEntity(pos);
            if (blockEntity instanceof EnderUpperBlockEntity) {
                ItemScatterer.spawn(world, pos, (EnderUpperBlockEntity)blockEntity);
    ...
    public void onEntityCollision(BlockState state, World world, BlockPos pos, Entity entity) {
        BlockEntity blockEntity = world.getBlockEntity(pos);
        if (blockEntity instanceof EnderUpperBlockEntity) {
            EnderUpperBlockEntity.onEntityCollided(world, pos, state, entity, (EnderUpperBlockEntity)blockEntity);
        }
