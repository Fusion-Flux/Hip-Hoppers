package com.fusionflux.hip_hoppers.blocks.NormalHoppers;
...
public class CustomHopperBlockEntity extends LootableContainerBlockEntity implements Hopper {
    ...
    public CustomHopperBlockEntity(BlockPos pos, BlockState state, int transferSpeed) {
        super(HipHoppersBlocks.CUSTOM_HOPPER_BLOCK_ENTITY,pos, state);
    ...
    private static boolean insertAndExtract(World world, BlockPos pos, BlockState state, CustomHopperBlockEntity blockEntity, BooleanSupplier booleanSupplier) {
        if (world.isClient) {
            return false;
        } else {
            if (!blockEntity.needsCooldown() && (Boolean)state.get(CustomHopperBlock.ENABLED)) {
                boolean bl = false;
    ...
    private static boolean insert(World world, BlockPos pos, BlockState state, Inventory inventory) {
        ...
        if (inventory2 == null) {
            ...
            Direction direction = ((Direction)state.get(CustomHopperBlock.FACING)).getOpposite();
    ...
    private static ItemStack transfer(@Nullable Inventory from, Inventory to, ItemStack stack, int slot, @Nullable Direction side) {
        ...
        if (canInsert(to, stack, slot, side)) {
            ...
            if (bl) {
                if (bl2 && to instanceof CustomHopperBlockEntity) {
                    CustomHopperBlockEntity hopperBlockEntity = (CustomHopperBlockEntity)to;
                    if (!hopperBlockEntity.isDisabled()) {
                        int j = 0;
                        if (from instanceof HopperBlockEntity) {
                            CustomHopperBlockEntity hopperBlockEntity2 = (CustomHopperBlockEntity)from;
    ...
    private static Inventory getOutputInventory(World world, BlockPos pos, BlockState state) {
        Direction direction = (Direction)state.get(CustomHopperBlock.FACING);
