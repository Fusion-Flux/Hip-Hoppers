...
public class UpperBlock extends BlockWithEntity {
    ...
    public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        return world.isClient ? null : checkType(type, HipHoppersBlocks.UPPER_BLOCK_ENTITY, UpperBlockEntity::serverTick);
    }

    public void onPlaced(World world, BlockPos pos, BlockState state, LivingEntity placer, ItemStack itemStack) {
        if (itemStack.hasCustomName()) {
            BlockEntity blockEntity = world.getBlockEntity(pos);
            if (blockEntity instanceof UpperBlockEntity) {
                ((UpperBlockEntity)blockEntity).setCustomName(itemStack.getName());
            }
        }
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        if (world.isClient) {
            ...
            BlockEntity blockEntity = world.getBlockEntity(pos);
            if (blockEntity instanceof UpperBlockEntity) {
                player.openHandledScreen((UpperBlockEntity)blockEntity);
    ...
    public void onStateReplaced(BlockState state, World world, BlockPos pos, BlockState newState, boolean moved) {
        if (!state.isOf(newState.getBlock())) {
            BlockEntity blockEntity = world.getBlockEntity(pos);
            if (blockEntity instanceof UpperBlockEntity) {
                ItemScatterer.spawn(world, pos, (UpperBlockEntity)blockEntity);
    ...
    public void onEntityCollision(BlockState state, World world, BlockPos pos, Entity entity) {
        BlockEntity blockEntity = world.getBlockEntity(pos);
        if (blockEntity instanceof UpperBlockEntity) {
            UpperBlockEntity.onEntityCollided(world, pos, state, entity, (UpperBlockEntity)blockEntity);
        }
