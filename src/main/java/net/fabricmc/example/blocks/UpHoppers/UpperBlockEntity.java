...
public class UpperBlockEntity extends LootableContainerBlockEntity implements Hopper {
    ...
    public static void serverTick(World world, BlockPos pos, BlockState state, UpperBlockEntity blockEntity) {
    ...
    private static boolean insertAndExtract(World world, BlockPos pos, BlockState state, UpperBlockEntity blockEntity, BooleanSupplier booleanSupplier) {
        if (world.isClient) {
            return false;
        } else {
            if (!blockEntity.needsCooldown() && (Boolean)state.get(UpperBlock.ENABLED)) {
                boolean bl = false;
    ...
    private static boolean insert(World world, BlockPos pos, BlockState state, Inventory inventory) {
        ...
        if (inventory2 == null) {
            ...
            Direction direction = ((Direction)state.get(UpperBlock.FACING)).getOpposite();
    ...
    private static ItemStack transfer(@Nullable Inventory from, Inventory to, ItemStack stack, int slot, @Nullable Direction side) {
        ...
        if (canInsert(to, stack, slot, side)) {
            ...
            if (bl) {
                if (bl2 && to instanceof UpperBlockEntity) {
                    UpperBlockEntity hopperBlockEntity = (UpperBlockEntity)to;
                    if (!hopperBlockEntity.isDisabled()) {
                        int j = 0;
                        if (from instanceof HopperBlockEntity) {
                            UpperBlockEntity hopperBlockEntity2 = (UpperBlockEntity)from;
    ...
    private static Inventory getOutputInventory(World world, BlockPos pos, BlockState state) {
        Direction direction = (Direction)state.get(UpperBlock.FACING);
    ...
    public static void onEntityCollided(World world, BlockPos pos, BlockState state, Entity entity, UpperBlockEntity blockEntity) {
