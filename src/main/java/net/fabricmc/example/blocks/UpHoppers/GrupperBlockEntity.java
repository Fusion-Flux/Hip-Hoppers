...
public class GrupperBlockEntity extends LootableContainerBlockEntity implements Hopper {
    ...
    public GrupperBlockEntity(BlockPos pos, BlockState state) {
        this(pos,state, 8);
    }

    public GrupperBlockEntity(BlockPos pos, BlockState state, int transferSpeed) {
        super(HipHoppersBlocks.GRUPPER_BLOCK_ENTITY,pos, state);
    ...
    public static void serverTick(World world, BlockPos pos, BlockState state, GrupperBlockEntity blockEntity) {
    ...
    private static boolean insertAndExtract(World world, BlockPos pos, BlockState state, GrupperBlockEntity blockEntity, BooleanSupplier booleanSupplier) {
        if (world.isClient) {
            return false;
        } else {
            if (!blockEntity.needsCooldown() && (Boolean)state.get(GrupperBlock.ENABLED)) {
                boolean bl = false;
    ...
    private static boolean insert(World world, BlockPos pos, BlockState state, Inventory inventory) {
        ...
        if (inventory2 == null) {
            ...
            Direction direction = ((Direction)state.get(GrupperBlock.FACING)).getOpposite();
    ...
    private static ItemStack transfer(@Nullable Inventory from, Inventory to, ItemStack stack, int slot, @Nullable Direction side) {
        ...
        if (canInsert(to, stack, slot, side)) {
            ...
            if (bl) {
                if (bl2 && to instanceof GrupperBlockEntity) {
                    GrupperBlockEntity hopperBlockEntity = (GrupperBlockEntity)to;
                    if (!hopperBlockEntity.isDisabled()) {
                        int j = 0;
                        if (from instanceof HopperBlockEntity) {
                            GrupperBlockEntity hopperBlockEntity2 = (GrupperBlockEntity)from;
    ...
    private static Inventory getOutputInventory(World world, BlockPos pos, BlockState state) {
        Direction direction = (Direction)state.get(GrupperBlock.FACING);
    ...
    public static void onEntityCollided(World world, BlockPos pos, BlockState state, Entity entity, GrupperBlockEntity blockEntity) {
