...
public class GrupperBlock extends BlockWithEntity {
    ...
    public GrupperBlock(Settings settings, int itemMoveSpeed) {
        super(settings);
    ...
    public BlockEntity createBlockEntity(BlockPos pos, BlockState state) {
        return new GrupperBlockEntity(pos, state,this.transferSpeed);
    }
    @Override
    ...
    public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        return world.isClient ? null : checkType(type, HipHoppersBlocks.GRUPPER_BLOCK_ENTITY, GrupperBlockEntity::serverTick);
    }

    public void onPlaced(World world, BlockPos pos, BlockState state, LivingEntity placer, ItemStack itemStack) {
        if (itemStack.hasCustomName()) {
            BlockEntity blockEntity = world.getBlockEntity(pos);
            if (blockEntity instanceof GrupperBlockEntity) {
                ((GrupperBlockEntity)blockEntity).setCustomName(itemStack.getName());
            }
        }
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        if (world.isClient) {
            ...
            BlockEntity blockEntity = world.getBlockEntity(pos);
            if (blockEntity instanceof GrupperBlockEntity) {
                player.openHandledScreen((GrupperBlockEntity)blockEntity);
    ...
    public void onStateReplaced(BlockState state, World world, BlockPos pos, BlockState newState, boolean moved) {
        if (!state.isOf(newState.getBlock())) {
            BlockEntity blockEntity = world.getBlockEntity(pos);
            if (blockEntity instanceof GrupperBlockEntity) {
                ItemScatterer.spawn(world, pos, (GrupperBlockEntity)blockEntity);
    ...
    public void onEntityCollision(BlockState state, World world, BlockPos pos, Entity entity) {
        BlockEntity blockEntity = world.getBlockEntity(pos);
        if (blockEntity instanceof GrupperBlockEntity) {
            GrupperBlockEntity.onEntityCollided(world, pos, state, entity, (GrupperBlockEntity)blockEntity);
        }
