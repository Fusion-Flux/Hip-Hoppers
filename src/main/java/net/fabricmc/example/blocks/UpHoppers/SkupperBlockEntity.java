...
public class SkupperBlockEntity extends LootableContainerBlockEntity implements Hopper {
    ...
    public SkupperBlockEntity(BlockPos pos, BlockState state) {
        this(pos,state, 8);
    }

    public SkupperBlockEntity(BlockPos pos, BlockState state, int transferSpeed) {
        super(HipHoppersBlocks.SKUPPER_BLOCK_ENTITY,pos, state);
    ...
    protected Text getContainerName() {
        return new TranslatableText("container.upper");
    }

    public static void serverTick(World world, BlockPos pos, BlockState state, SkupperBlockEntity blockEntity) {
    ...
    private static boolean insertAndExtract(World world, BlockPos pos, BlockState state, SkupperBlockEntity blockEntity, BooleanSupplier booleanSupplier) {
        if (world.isClient) {
            return false;
        } else {
            if (!blockEntity.needsCooldown() && (Boolean)state.get(SkupperBlock.ENABLED)) {
                boolean bl = false;
    ...
    private static boolean insert(World world, BlockPos pos, BlockState state, Inventory inventory) {
        ...
        if (inventory2 == null) {
            ...
            Direction direction = ((Direction)state.get(SkupperBlock.FACING)).getOpposite();
    ...
    private static ItemStack transfer(@Nullable Inventory from, Inventory to, ItemStack stack, int slot, @Nullable Direction side) {
        ...
        if (canInsert(to, stack, slot, side)) {
            ...
            if (bl) {
                if (bl2 && to instanceof SkupperBlockEntity) {
                    SkupperBlockEntity hopperBlockEntity = (SkupperBlockEntity)to;
                    if (!hopperBlockEntity.isDisabled()) {
                        int j = 0;
                        if (from instanceof HopperBlockEntity) {
                            SkupperBlockEntity hopperBlockEntity2 = (SkupperBlockEntity)from;
    ...
    private static Inventory getOutputInventory(World world, BlockPos pos, BlockState state) {
        Direction direction = (Direction)state.get(SkupperBlock.FACING);
    ...
    public static void onEntityCollided(World world, BlockPos pos, BlockState state, Entity entity, SkupperBlockEntity blockEntity) {
